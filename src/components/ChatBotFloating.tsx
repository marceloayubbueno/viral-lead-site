...
import styles from './ChatBotFloating.module.css';
...
const ChatBotFloating = () => {
  ...
  console.log('ChatBotFloating RENDER', { windowWidth: typeof window !== 'undefined' ? window.innerWidth : 'ssr' });

  useEffect(() => {
    const el = document.querySelector('.' + styles.chatbotContainer.replace(/\./g, ''));
    if (el) {
      console.log('ChatBotFloating container size:', el.getBoundingClientRect());
    } else {
      console.log('ChatBotFloating container NOT FOUND');
    }
  }, []);
